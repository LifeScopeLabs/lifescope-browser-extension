<template>
  <button v-if="domainWhitelisted" class="danger" v-on:click="deleteWhitelistEntry">Stop tracking this domain</button>
  <button v-else class="primary" v-on:click="addWhitelistEntry">Start tracking this domain</button>
</template>
    ...
    export default {
	    ...
	    methods: {
		    addWhitelistEntry: async function() {
			    ...
			    let domainExists = this.$store.state.whitelist.indexOf(this.$data.domain);
